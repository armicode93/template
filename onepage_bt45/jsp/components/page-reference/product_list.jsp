...
%><c:if	test="${fn:length(pages)>0}">
	...
	<c:if test="${fn:length(taxonomyList)>0}">
		<div class="taxonomy"><c:forEach var="taxo" items="${taxonomyList}">
			<div class="label label-default name-${taxo.name}">${taxo.label}</div>
			</c:forEach></div>
	</c:if>
	...
	<div class="products row justify-content-center">
		<c:forEach items="${pages}" var="page" varStatus="status">
			...
			<div class="col item-${status.index+1} ${status.index%2==0?'odd':'even'} ${page.toTheTopLevel>0?'top':''}${cssClass}" ${style}>
				<div class="item">
					<c:if test="${not empty image}"><div class="thumbnail">												
						<figure>
							<a title="${page.title}" href="${page.linkOn}" class="js-scroll-trigger">
					...
					<div class="caption">					
						...
						<h3><a title="${page.title}" href="${page.linkOn}">${page.title}</a></h3>
