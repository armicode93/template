		...
		<c:choose>	
			<c:when test="${field.type eq 'text' || field.type eq 'email' || field.type eq 'vat'}">
				<div class="form-group${not empty errorFields[field.name]?' has-error':''} has-feedback label-size-${field.labelSize}">
					...
					<input class="form-control" ${autocomplete}type="text" name="${field.name}" id="${field.name}" value="${requestService.parameterForAttributeMap[field.name]}" placeholder="${field.label}${field.require?'*':''}" ${field.require?requireInput:''} />
			...
			<c:when test="${field.type eq 'number'}">
				<div class="form-group${not empty errorFields[field.name]?' has-error':''} has-feedback label-size-${field.labelSize}">
					<label class="control-label" for="${field.name}">${field.label} ${field.require?requireHTML:''}</label>				
					<input class="form-control" type="number" ${autocomplete}name="${field.name}" id="${field.name}" value="${requestService.parameterForAttributeMap[field.name]}" placeholder="${field.label}${field.require?'*':''}" ${field.require?requireInput:''} />
					<c:if test="${not empty errorFields[field.name]}"><span class="glyphicon glyphicon-remove form-control-feedback" aria-hidden="true"></span></c:if>
				</div>
			</c:when>
			<c:when test="${field.type eq 'large-text'}">
				<div class="form-group${not empty errorFields[field.name]?' has-error':''} has-feedback label-size-${field.labelSize}">
					<label class="control-label" for="${field.name}">${field.label} ${field.require?requireHTML:''}</label>
					<textarea class="form-control" ${autocomplete}name="${field.name}" id="${field.name}" placeholder="${field.label}${field.require?'*':''}" ${field.require?requireInput:''}>${requestService.parameterForAttributeMap[field.name]}</textarea>
					<c:if test="${not empty errorFields[field.name]}"><span class="glyphicon glyphicon-remove form-control-feedback" aria-hidden="true"></span></c:if>
				</div>
			</c:when>
