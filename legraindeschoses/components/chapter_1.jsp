...
<c:if test="${empty param.sommaire}">
	...
	<h3><a class="play" href="#" onclick="selectChapter(${soundNumber}, true, '${sommaireURL}'); return false;" title="jouer le morceau"><jv:texttohtml>${title.value}</jv:texttohtml></a>
		<c:if test="${not empty information.linkLabel}"><a class="info-link ajax-nav-done" title="${information.linkLabel}" href="${information.link}"><img src="${info.rootTemplateURL}/img/info.png" alt="info" /></a></c:if>
