...
function setCookie(cname, cvalue, exdays) {
    ...
    document.cookie = encodeCookie(cname) + "=" + cvalue + ";" + expires + ";path=/";
} 
...
function deleteCookie( cname ) {	
    document.cookie = encodeURI(cname) + '=; expires=Thu, 01 Jan 1970 00:00:01 GMT;';
}
...
function firstLoad() {
	...
	timeline.addEventListener("click", function(e) {
		...
		music.currentTime = (e.clientX-rect.left)/(rect.right-rect.left)*music.duration;
	});
...
<c:if test="${not empty summaryUrl}"><div id="btn-summary"><a class="btn btn-secondary btn-sm" data-toggle="collapse" href="#sound-summary-wrapper" role="button" aria-expanded="false" aria-controls="sound-summary-wrapper">
<i class="fas fa-angle-double-up collapse-on"></i><i class="fas fa-angle-double-down collapse-off"></i> Sommaire <i class="fas fa-angle-double-up collapse-on"></i><i class="fas fa-angle-double-down collapse-off"></i>
</a></div></c:if>
<div id="sound-player" class="sound-player ajax-nav">	
	<div id="audioplayer" class="audioPlayer">
		...
		<div class="command">
			<button id="prevButton" class="previous" onclick="changeSound(false)" title="morceau prÃƒÂ©cÃƒÂ©dant"><img src="${info.rootTemplateURL}/img/left.png" /></button>
			<button id="pButton" class="play" onclick="playAudio()" title="jouer le morceau">&nbsp;</button>
			<button id="nextButton" class="next" onclick="changeSound(true)" title="morcceau suivant"><img src="${info.rootTemplateURL}/img/right.png" /></button>
		</div>
	</div>
	<div class="time start-time" id="start-time"></div>
	<div class="time end-time" id="end-time"></div>
	<div class="title-block row">
		<div class="col-md-12 text-center"><div id="sound-title" class="titlee">${staticInfoHTML}</div></div>
	</div>
    <div class="row">
		<div class="col-6 text-left"><a href="<jv:pageurl name='sommaires-1'/>">j'&eacute;coute le n&deg;1</a></div>		
		<div class="col-6 text-right"><a href="<jv:pageurl name='sommaires-2'/>" id="sommaire-link">j'&eacute;coute le n&deg;2</a></div>
	</div>
	
	<div id="sommaire" style="display: none;"></div>
	<script>
		<c:if test="${empty sommaireURL}">
		<jv:pageurl var="JvsommaireURL" name='sommaires-2' />
		...
		function loadSommaire(sommaireURL) {
			...
			xhttp.onreadystatechange = function() {
		 		if (this.readyState == 4 && this.status == 200) {
		 			console.log("load sommaire : "+sommaireURL);
					document.getElementById("sommaire").innerHTML = this.response;
					firstLoad();
				} else {
					console.log("error log sommaire : "+sommaireURL);
				}
			};
			xhttp.open("GET", sommaireURL, true);
			xhttp.send();
		}
		loadSommaire('${sommaireURL}');	 
	</script>
